...
Customer customer=custRepo.findById(username).orElse(null);
	...
	public String updatePassword(AuthRequest authRequest) {
		Customer customer=custRepo.findById(authRequest.getUsername()).orElse(null);
		UpdateProfileDTO updateProfileDTO=new UpdateProfileDTO();	
			if(customer!=null) {
				customer.setPassword(passwordEncoder.encode(authRequest.getNewPassword()));
				custRepo.save(customer);
			...
			Admin admin=adminRepository.findById(authRequest.getUsername()).orElse(null);
			if(admin!=null) {
				admin.setPassword(passwordEncoder.encode(authRequest.getNewPassword()));
				adminRepository.save(admin);
				return "password updated successfully";
			}
			
			
			throw new UsernameNotFoundException("username not found with"+authRequest.getUsername());
	}
}
